                    ...
                    if (hasParams) {
                        ...
                        modalRefs.submitBtn.onclick = async () => {
                            ...
                            inputs.forEach(input => {
                                ...
                                // DECODE KHUSUS UNTUK PARAMETER BERNAMA 'url'
                                if (paramName === 'url' && paramValue) {
                                    try {
                                        // Decode URL yang ter-encode
                                        paramValue = decodeURIComponent(paramValue);

                                        // Validasi format URL setelah decode
                                        if (!paramValue.startsWith('http://') && !paramValue.startsWith('https://')) {
                                            throw new Error('Format URL tidak valid');
                                        }
                                    } catch (e) {
                                        // Tampilkan pesan error ke user
                                        showToast('URL tidak valid: ' + e.message, 'error');
                                        isValid = false;
